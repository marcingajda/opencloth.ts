...
class Ellipsoid{
    ...
    matrix: TSM.mat4;
    matrixInverse: TSM.mat4;

    iStacks: number;
    iSlices: number;
    fRadius: number;

    public constructor(iStacks: number, iSlices: number, fRadius: number){
    ...
    public collision(points: Particle[]) {
        ...
        for (var i = 0; i < points.length; i++) {
            ...
            var predictionVec4:TSM.vec4 = new TSM.vec4([points[i].prediction.x, points[i].prediction.y, points[i].prediction.z, 1]);
            var x0:TSM.vec4 = this.matrixInverse.copy().multiplyVec4(predictionVec4);

            // glm::vec3 delta0 = glm::vec3(X_0.x, X_0.y, X_0.z) - center;
            var delta0:TSM.vec3 = (new TSM.vec3([x0.x, x0.y, x0.z])).subtract(new TSM.vec3([0,0,0]));

            var distance:number = delta0.length();

            var delta:TSM.vec3 = TSM.vec3.zero.copy();
            var transformInv:TSM.vec3;
